...
AudioPlayer::AudioPlayer(std::function<void(TransportState)> state_caller)
    : state{Stopped}, state_callback{state_caller} {
  format_manager.registerBasicFormats();
  transport_source.addChangeListener(this);
  setAudioChannels(0, 2);
}

AudioPlayer::TransportState AudioPlayer::getState() { return state; }

void AudioPlayer::changeState(TransportState new_state) {
  if (state != new_state) {
    state = new_state;
    switch (state) {
      case Stopped:
